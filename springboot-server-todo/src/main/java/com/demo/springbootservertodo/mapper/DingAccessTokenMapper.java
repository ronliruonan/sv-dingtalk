...
public interface DingAccessTokenMapper {
    ...
    @Insert({"insert into ding_access_token(app_key,access_token,begin_time) values(#{app_key},#{access_token},#{begin_time})"})
    void insert(DingAccessToken accessToken);

    @Update({"update ding_access_token set access_token=#{access_token}, begin_time=#{begin_time} where app_key=#{app_key}"})
